...
select min(maxi) from (select dept_name, max(salary) from instructor i group by dept_name) as max_salary(dept_name, maxi);
...
select distinct A.course_id, A.id from (select ID, course_id, count(cohttps://www.google.com/url?sa=i&url=https%3A%2F%2Fscipy-lectures.org%2Fintro%2Fnumpy%2Farray_object.html&psig=AOvVaw1rPjmoodCYbXOMu478tWQI&ust=1665021624565000&source=images&cd=vfe&ved=0CAwQjRxqFwoTCLC7i4z_x_oCFQAAAAAdAAAAABADurse_id) as taken_count from takes group by ID,course_id) as A where A.taken_count >= 3 ORDER BY A.course_id;
...
select name from faculty where dept_name='Comp. Sci';

create view dept_sal_tot as select * from instructor gorup group by dept_name;

select * from dept_sal_tot; 	#sum이 아닌 각 나옴.
...
drop view dept_tot_sal;

create view physics_fall_2009 as select course.course_id, sec_id, building
from course, section where course.course_id = section.course_id and
	course.dept_name = 'Physics' and section.semester = 'Fall'
...
create view CS_Spring_2009 as select course.course_id, sec_id, building
from course, section where course.course_id = section.course_id and
	course.dept_name = 'Comp. Sci' and section.semester = 'Spring'
	and section.year = '2009';
    
select * from CS_Spring_2009;

create view CS_Spering_2009_IT as select course.course_id, sec_id, building
from course, section where course.course_id = section.course_id and
	course.dept_name = 'Comp. Sci' and section.semester = 'Spring'
	and section.year = '2009' and building = 'IT';

select * from CS_Spering_2009_IT;

create view CS_spring_2009_IT2 as select * from CS_Spring_2009 where building = 'IT';

select * from CS_spring_2009_IT2;